...
lthnSDPHttpRequestInput::lthnSDPHttpRequestInput() {
    initialize();
}

lthnSDPHttpRequestInput::lthnSDPHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void lthnSDPHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void lthnSDPHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void lthnSDPHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    lthnSDPHttpFileElement file;
...
lthnSDPHttpRequestWorker::lthnSDPHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
lthnSDPHttpRequestWorker::~lthnSDPHttpRequestWorker() {
...
QMap<QString, QString> lthnSDPHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

lthnSDPHttpFileElement lthnSDPHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return lthnSDPHttpFileElement();
}

QByteArray *lthnSDPHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void lthnSDPHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void lthnSDPHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void lthnSDPHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void lthnSDPHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  lthnSDPHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString lthnSDPHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void lthnSDPHttpRequestWorker::execute(lthnSDPHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<lthnSDPHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, lthnSDPCompressionType::Gzip));
        } else {
    ...
    if (lthnSDPHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*lthnSDPHttpRequestWorker::sslDefaultConfiguration);
    }
...
void lthnSDPHttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void lthnSDPHttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void lthnSDPHttpRequestWorker::process_response(QNetworkReply *reply) {
    ...
    if (!contentDispositionHdr.isEmpty()) {
        ...
        if ((contentDisposition.count() > 0) && (contentDisposition.first() == QString("attachment"))) {
            ...
            lthnSDPHttpFileElement felement;
...
QByteArray lthnSDPHttpRequestWorker::decompress(const QByteArray& data){
    QByteArray result;
    bool sts = false;
    do{
        z_stream strm{};
...
QByteArray lthnSDPHttpRequestWorker::compress(const QByteArray& input, int level, lthnSDPCompressionType compressType) {
    QByteArray output;
    static const int GZIP_WINDOW_BIT = 15+16;
    static const int ZLIB_WINDOW_BIT = 15;
    static const int CHUNK_SIZE = 8*1024;
    int windowBits;
    if( compressType == lthnSDPCompressionType::Gzip ){
        windowBits = GZIP_WINDOW_BIT;
    } else if ( compressType == lthnSDPCompressionType::Zlib ){
...
QSslConfiguration *lthnSDPHttpRequestWorker::sslDefaultConfiguration;
